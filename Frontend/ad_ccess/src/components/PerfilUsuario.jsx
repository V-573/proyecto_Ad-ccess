      ...
      <main className="layout-content-custom">
        <div className="perfil-card">
            ...
            <div className="perfil-titulos">
              ...
              <div className="acciones-perfil">
                <button
                  ...
                  onClick={() => navigate(`/registrar-usuario/${perfil.id}`)}
                >
          ...
          <section className="info-basica">
            ...
            {perfil.casa_apto && (
              <p>
                <strong>Casa/Apto:</strong> {perfil.casa_apto}
              </p>
            )}
          </section>
              ...
              <div className="familia-grid">
                {perfil.familiares?.length > 0 ? (
                  perfil.familiares.map((f) => (
                    <div
                      key={f.id || f.identificacion}
                      className="familiar-card-mini"
                    >
                      <img
                        src={
                          f.foto
                            ? `http://localhost:4000${f.foto}`
                            : "/default-avatar.png"
                        }
                        alt={f.nombre}
              ...
              <ul className="vehiculos-lista">
                {perfil.vehiculos?.length > 0 ? (
                  perfil.vehiculos.map((v) => (
                    <li key={v.id || v.placa}>
                      <i className="ph ph-car"></i> {v.placa} - {v.tipo} (
                      {v.color})
                    </li>
                  ))
                ) : (
